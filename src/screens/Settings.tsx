...
export default function SettingsScreen() {
  ...
  const displayedDailyGoal = convertFromOunces(settings.dailyIntakeGoal, settings.measurementUnit);

  const handleGoalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newGoalInUnit = parseFloat(e.target.value);
    const newGoalInOunces = convertToOunces(newGoalInUnit, settings.measurementUnit); // Convert to ounces before dispatching
    updateDailyIntakeGoal(newGoalInOunces);
  };

  return (
    <div className={`relative flex flex-col items-center justify-center h-screen font-sans ${theme === "dark" ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
      <ScrollArea className="w-full max-w-lg mx-auto p-4 h-full pt-16">
        ...
        <div className="relative z-10 flex flex-col items-center w-full space-y-8">
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <Select
              ...
              onValueChange={(value: "oz" | "mL" | "L" | "cups") => updateMeasurementUnit(value)}
            >
        ...
        <div className="relative z-10 flex flex-col items-center w-full space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
            <Button variant="secondary" onClick={backupSettingsData} disabled={true}>
              <FileDown className="mr-2 h-4 w-4" />
              Export Data (CSV)
            </Button>
            <Button variant="secondary" onClick={loadSettingsFromCSV} disabled={true}>
              <FileUp className="mr-2 h-4 w-4" />
              Load Data (CSV)
            </Button>
            <Button variant="destructive" onClick={clearAllHistory} disabled={true}>
