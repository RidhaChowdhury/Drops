...
export default function Log({ isActive }: { isActive: boolean }) {
   ...
   const measurementUnit = useSelector((state: RootState) => state.settings.measurementUnit);
   const currentDate = new Date().toISOString().split("T")[0];
   const waterHistory = getWaterHistory();

   const todayEntry = waterHistory.find((entry) => entry.date === currentDate);
   const [waterIntake, setWaterIntake] = useState(todayEntry ? todayEntry.intake : 0);
   const [drinkLog, setDrinkLog] = useState(todayEntry && todayEntry.drinkLog ? todayEntry.drinkLog : []);

   const [quickAddValues, setQuickAddValues] = useState(() => JSON.parse(localStorage.getItem("quickAddValues") || "[8, 16]"));
   ...
   useEffect(() => {
      const updatedHistory = waterHistory.filter((entry) => entry.date !== currentDate);
   ...
   const handleRightClickQuickAdd = (e: React.MouseEvent<HTMLButtonElement>, index: number) => {
      e.preventDefault();
   ...
   const handleSaveQuickAdd = () => {
      const newQuickAddValueInOz = convertToOunces(newQuickAddValue, measurementUnit);
   ...
   const handleDeleteQuickAdd = () => {
      const newValues = quickAddValues.filter((_: any, index: number) => index !== currentButton);
      setQuickAddValues(newValues);
      setIsQuickAddDrawerOpen(false);
   };
   ...
   return (
      <div className={`relative flex flex-col items-center justify-center min-h-screen overflow-hidden font-sans ${theme === "dark" ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
         ...
         <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-6 mb-8">
               {quickAddValues.sort((a: number, b: number) => a - b).map((value: number, index: number) => (
                  <Button
                     key={index}
               ...
               <Button onClick={handleAddNewQuickAdd} variant="secondary" className="rounded-2xl text-2xl h-16 w-16 flex items-center justify-center">
                  <Plus />
               </Button>
            </div>
         </div>
         ...
         <WaterEntryDrawer
            ...
            onDecrease={() => setNewQuickAddValue((prev) => Math.max(1, prev - 1))}
